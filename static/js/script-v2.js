function drawOrganizationChart(params) {
    ...
    function getFeature(colorCategory) {
        var parameter = '';
        switch(parseInt(colorCategory)) {
    ...
    var colorCategoryRank = {
        'High' : 3,
        'Medium' : 2,
        'Low' : 1
    }

    function sortByMetric(first, second) {
        var metric = getFeature(params.colorBy);
        ...
        if (params.colorBy <= 3) {
            value1 = first[metric];
            value2 = second[metric];
        }
        else {
            value1 = colorCategoryRank[first[metric]];
            value2 = colorCategoryRank[second[metric]];
        }
	...
	function sortData(node, depth) {
		...
		if(node.hasOwnProperty('children')) {
            if (params.colorBy != 0) {
                node['children'].sort(sortByMetric)
            }

			for(var i=0; i<node.children.length; i+=1) {
				sortData(node['children'][i], depth + 1);
			}
		}
    }
    ...
    function filterNodes(node) {
        if (node.hasOwnProperty('children')) {
            ...
            for(var i=0; i<size; i+=1) {
                if(OrgTypeLevel[node['children'][i].ENTITY_ORG_TYPE] > selectedEntityLevel) {
                    node['children'].splice(i, 1);
                    i -= 1;
                    size -= 1;
                }
                else {
                    filterNodes(node['children'][i])
                }
            }
    ...
    sortData(attrs.root, 0)
    if (selectedEntityLevel != 6) {
        filterNodes(attrs.root);
    }
...
//                return toTitleCase(d.ENTITY_NAME.toLowerCase()).trim();
        ...
        parents.forEach(function(p) {
            ...
            pageControl
              	...
              	.attr("xlink:href", function(d) {
                	if (d.type == "next") { return "./static/svg/next_2.svg" } 
					else { return "./static/svg/prev_2.svg" }
              	})
        ...
        function sideBarContent(item) {
            ...
            var strVar = '';
            strVar += `<div class="ui sidebarcard">`
            strVar += `		<div class="detailsBar">`
        ...
        function sideBarHandler(d) {
            ...
            if (selectedNodeId != "") {
				d3.select(selectedNodeId).style("stroke-width", dimens.nodeStrokeWidth)
...
//	        selectedOriginalBackground = d3.select(selectedNodeId).attr("fill")
            ...
            d3.select(selectedNodeId).style("stroke-width", dimens.selectedNodeStrokeWidth)
    ...
    function locate(id, recursive = true) {
		...
		selectedNodeId = ("#N" + id)
        d3.select(selectedNodeId).style("stroke-width", dimens.selectedNodeStrokeWidth)
        //d3.select(selectedNodeId).style("stroke", colors.nodeStroke)
